...
fn jpy_eur_long() -> Position<Runtime> {
	Position {
		owner: ALICE,
		pool: MOCK_POOL,
		pair: JPY_EUR_PAIR,
...
fn jpy_eur_short() -> Position<Runtime> {
	Position {
		owner: ALICE,
		pool: MOCK_POOL,
		pair: JPY_EUR_PAIR,
...
fn unrealized_pl_of_long_position_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(
				MarginProtocol::_unrealized_pl_of_position(&jpy_eur_long()),
...
fn unrealized_pl_of_short_position_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(
				MarginProtocol::_unrealized_pl_of_position(&jpy_eur_short()),
...
fn unrealized_pl_of_trader_sums_all_positions() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			<Positions<Runtime>>::insert(0, jpy_eur_long());
			<Positions<Runtime>>::insert(1, jpy_eur_short());
...
fn margin_level_without_any_opened_position_is_max() {
	ExtBuilder::default().build().execute_with(|| {
		<Balances<Runtime>>::insert(ALICE, MOCK_POOL, Fixed128::from_parts(1));
		assert_eq!(
			MarginProtocol::margin_level(&ALICE, MOCK_POOL),
...
fn open_long_position_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				MarginProtocol::positions_by_pool(MOCK_POOL, (JPY_EUR_PAIR, id)),
				Some(true)
			);
...
fn open_position_fails_if_would_reach_enp_threshold() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			LiquidityPoolENPThreshold::insert(JPY_EUR_PAIR, risk_threshold(10, 5));
			<Balances<Runtime>>::insert(ALICE, MOCK_POOL, fixed128_from_natural_currency_cent(10_000_00));
			assert_noop!(
				MarginProtocol::open_position(
					Origin::signed(ALICE),
...
fn open_position_fails_if_would_reach_ell_threshold() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			LiquidityPoolELLThreshold::insert(JPY_EUR_PAIR, risk_threshold(10, 5));
			<Balances<Runtime>>::insert(ALICE, MOCK_POOL, fixed128_from_natural_currency_cent(10_000_00));
			assert_noop!(
				MarginProtocol::open_position(
					Origin::signed(ALICE),
...
fn close_loss_position_realizing_nothing_on_negative_equity() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(MarginProtocol::balances(&ALICE, MOCK_POOL), Fixed128::from_natural(-88));
		});
}
...
fn deposit_works() {
	ExtBuilder::default().alice_balance(1000).build().execute_with(|| {
		...
		assert_eq!(MarginProtocol::balances(&ALICE, MOCK_POOL), Fixed128::from_parts(500));
...
fn liquidity_pool_manager_can_remove_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		<Positions<Runtime>>::insert(0, jpy_eur_long());
		PositionsByPool::insert(MOCK_POOL, (JPY_EUR_PAIR, 0), true);
		assert!(!<MarginProtocol as LiquidityPoolManager<LiquidityPoolId, Balance>>::can_remove(MOCK_POOL));
	});
}
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				MarginProtocol::_enp_and_ell(MOCK_POOL, Action::OpenPosition(eur_usd_long_1())),
				Ok((
					Fixed128::from_parts(89092827372129117),
					Fixed128::from_parts(89092827372129117)
				))
			);
...
fn get_trader_max_threshold_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			TraderRiskThreshold::insert(JPY_EUR_PAIR, risk_threshold(5, 3));
			...
			assert_eq!(
				MarginProtocol::_get_trader_max_threshold(&ALICE, Action::None),
				RiskThreshold {
					margin_call: Permill::from_percent(5),
					stop_out: Permill::from_percent(5),
				}
			);

			assert_eq!(
				MarginProtocol::_get_trader_max_threshold(&ALICE, Action::OpenPosition(eur_usd_long_1())),
				RiskThreshold {
					margin_call: Permill::from_percent(5),
					stop_out: Permill::from_percent(5),
				}
			...
			assert_eq!(
				MarginProtocol::_get_trader_max_threshold(&ALICE, Action::OpenPosition(jpy_usd_long_1())),
				RiskThreshold {
					margin_call: Permill::from_percent(6),
					stop_out: Permill::from_percent(7),
				}
			);
		});
}
...
fn get_liquidity_pool_max_threshold_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			LiquidityPoolENPThreshold::insert(JPY_EUR_PAIR, risk_threshold(5, 3));
			LiquidityPoolENPThreshold::insert(JPY_USD_PAIR, risk_threshold(6, 7));

			<Positions<Runtime>>::insert(0, eur_usd_long_1());
			<Positions<Runtime>>::insert(1, jpy_eur_short());
			PositionsByPool::insert(MOCK_POOL, (EUR_USD_PAIR, 0), true);
			PositionsByPool::insert(MOCK_POOL, (JPY_EUR_PAIR, 1), true);

			assert_eq!(
				MarginProtocol::_get_liquidity_pool_max_threshold(MOCK_POOL, Action::None),
				(
					RiskThreshold {
						margin_call: Permill::from_percent(5),
						stop_out: Permill::from_percent(5),
					...
					RiskThreshold {
						margin_call: Permill::from_percent(0),
						stop_out: Permill::from_percent(0)
					}
				)
			);

			assert_eq!(
				MarginProtocol::_get_liquidity_pool_max_threshold(MOCK_POOL, Action::OpenPosition(eur_usd_long_1())),
				(
					RiskThreshold {
						margin_call: Permill::from_percent(5),
						stop_out: Permill::from_percent(5),
			...
			assert_eq!(
				MarginProtocol::_get_liquidity_pool_max_threshold(MOCK_POOL, Action::OpenPosition(jpy_usd_long_1())),
				(
					RiskThreshold {
						margin_call: Permill::from_percent(6),
						stop_out: Permill::from_percent(7),
