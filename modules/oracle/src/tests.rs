...
#[test]
fn should_add_operator() {
	new_test_ext().execute_with(|| {
		assert_ok!(ModuleOracle::add_operator(system::RawOrigin::Root.into(), 1));
		assert_eq!(ModuleOracle::operators(), vec!(1));
	});
}

#[test]
fn should_remove_operator() {
	new_test_ext().execute_with(|| {
		assert_ok!(ModuleOracle::add_operator(system::RawOrigin::Root.into(), 1));
		assert_eq!(ModuleOracle::operators(), vec!(1));
		assert_ok!(ModuleOracle::remove_operator(system::RawOrigin::Root.into(), 1));
		assert_eq!(ModuleOracle::operators(), vec!());
	});
}

#[test]
#[should_panic]
fn should_panic_when_operator_doesnt_exists() {
	new_test_ext().execute_with(|| {
		let _ = ModuleOracle::remove_operator(system::RawOrigin::Root.into(), 1);
	});
}
...
fn should_read_raw_values() {
	new_test_ext().execute_with(|| {
		assert_ok!(ModuleOracle::add_operator(system::RawOrigin::Root.into(), 1));
		assert_ok!(ModuleOracle::add_operator(system::RawOrigin::Root.into(), 2));
		assert_ok!(ModuleOracle::add_operator(system::RawOrigin::Root.into(), 3));
